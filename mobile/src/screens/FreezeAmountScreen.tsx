...
function makeFreezeStyles(t: ThemeTokens) {
  return {
...
export function FreezeAmountScreen() {
  const tokens = useTheme();
  const styles = useMemo(() => StyleSheet.create(makeFreezeStyles(tokens) as Record<string, object>), [tokens]);
  const insets = useSafeAreaInsets();
  const language = useAppStore((s) => s.language);
  const availableBalance = useAppStore((s) => s.availableBalance);
  const setOverlay = useAppStore((s) => s.setOverlay);
  const freezeAmount = useAppStore((s) => s.freezeAmount);
  ...
  const handleFinalConfirm = async () => {
    ...
    setError(null);
    const result = await freezeAmount({
      amount: numAmount,
      title: purpose.trim() || undefined,
      category: category.trim() || undefined,
    });
    if (result.success === false) {
      setError(result.error ?? i18n.t('quickPayment.insufficientBalance'));
      return;
    }
    setShowSuccess(true);
    ...
    return (
      <View style={[styles.successRoot, { paddingTop: insets.top }]}>
        <View style={styles.successGradient}>
          ...
          >
            <Lock size={48} color={tokens.primary} />
          </Animated.View>
          <Text style={styles.successTitle}>{i18n.t('freeze.success')}</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      >
        <View style={styles.header}>
          <Text style={styles.headerTitle}>{i18n.t('freeze.title')}</Text>
          <Pressable
        ...
        >
          <View style={styles.balanceCard}>
            <Text style={styles.balanceLabel}>
              {i18n.t('wallet.availableBalance')}
            </Text>
          ...
          <Text style={styles.inputLabel}>{i18n.t('freeze.amount')}</Text>
          ...
          <Text style={styles.inputLabel}>{i18n.t('freeze.purpose')}</Text>
          <TextInput
            style={styles.receiverInput}
            value={purpose}
            onChangeText={setPurpose}
            placeholder={i18n.t('freeze.purposePlaceholder')}
            placeholderTextColor={tokens.textMuted}
            maxLength={100}
          />

          <Text style={styles.inputLabel}>{i18n.t('freeze.category')}</Text>
          <TextInput
            style={styles.receiverInput}
            value={category}
          ...
          {(error || (numAmount > available && numAmount > 0)) && (
            <View style={styles.errorBox}>
              <Text style={styles.errorText}>
                {error ?? i18n.t('quickPayment.insufficientBalance')}
              </Text>
            </View>
          )}
        </ScrollView>
      ...
      {showConfirm && (
        ...
        >
          <Animated.View
            ...
            style={styles.modal}
          >
            <Text style={styles.modalTitle}>{i18n.t('freeze.confirmTitle')}</Text>
            <Text style={styles.modalMessage}>
              {i18n.t('freeze.confirmMessage')}
            </Text>
            <View style={styles.modalSummary}>
              <View style={styles.modalSummaryRow}>
                <Text style={styles.modalSummaryLabel}>
                  {i18n.t('freeze.amount')}
                </Text>
              ...
              <View style={styles.modalSummaryRow}>
                <Text style={styles.modalSummaryLabel}>
                  {i18n.t('freeze.purpose')}
                </Text>
                <Text style={styles.modalSummaryValue}>{purpose.trim()}</Text>
              </View>
            </View>
