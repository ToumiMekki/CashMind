...
import { X, Unlock } from 'lucide-react-native';
...
function makeUnfreezeStyles(t: ThemeTokens) {
  return {
...
export function UnfreezeAmountScreen() {
  const tokens = useTheme();
  const styles = useMemo(() => StyleSheet.create(makeUnfreezeStyles(tokens) as Record<string, object>), [tokens]);
  const insets = useSafeAreaInsets();
  const language = useAppStore((s) => s.language);
  const totalFrozenBalance = useAppStore((s) => s.totalFrozenBalance);
  const setOverlay = useAppStore((s) => s.setOverlay);
  const unfreezeAmount = useAppStore((s) => s.unfreezeAmount);
  ...
  const handleConfirm = async () => {
    setError(null);
    if (!isValid) return;
    const result = await unfreezeAmount(numAmount);
    if (result.success === false) {
      setError(result.error ?? i18n.t('unfreeze.insufficient'));
      return;
    }
    setShowSuccess(true);
      ...
      <View style={[styles.successRoot, { paddingTop: insets.top }]}>
        <View style={styles.successGradient}>
          <Animated.View entering={FadeInDown.duration(400)} style={styles.successIcon}>
            <Unlock size={48} color={tokens.primary} />
          </Animated.View>
          <Text style={styles.successTitle}>{i18n.t('unfreeze.success')}</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      >
        <View style={styles.header}>
          <Text style={styles.headerTitle}>{i18n.t('unfreeze.title')}</Text>
        ...
        >
          ...
          <Text style={styles.inputLabel}>{i18n.t('unfreeze.amount')}</Text>
          ...
          {(error || (numAmount > frozen && numAmount > 0)) && (
            <View style={styles.errorBox}>
              <Text style={styles.errorText}>
                {error ?? i18n.t('unfreeze.insufficient')}
              </Text>
            </View>
          )}
        </ScrollView>
